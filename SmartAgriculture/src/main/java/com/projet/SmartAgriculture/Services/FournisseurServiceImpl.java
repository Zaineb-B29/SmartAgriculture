...
import com.projet.SmartAgriculture.Repository.ConfirmationTokenRepository;
...
public class FournisseurServiceImpl implements FournisseurService {
    ...
    @Autowired
    EmailFournisseurService emailFournisseurService;

    @Autowired
    ConfirmationTokenRepository confirmationTokenRepository;

    @Override
    public ResponseEntity<Object> AjouterFournisseur(Fournisseur fournisseur) {
        Fournisseur existingUser = fournisseurRepository.findFournisseurByEmail(fournisseur.getEmail());
        ...
        ConfirmationToken confirmationToken = new ConfirmationToken(fournisseur);
        confirmationTokenRepository.save(confirmationToken);


        // Construction du message HTML avec un bouton de vérification
        String confirmationLink = "http://localhost:8081/api/fournisseur/confirm-account?token=" + confirmationToken.getConfirmationToken();
        ...
        String emailContent = "<!DOCTYPE html>" +
                ...
                "</div>" +
                "<p> Salut <strong>"+fournisseur.getNom()+"</strong>"+
        ...
        MimeMessage message = emailFournisseurService.createMimeMessage();
        ...
        emailFournisseurService.SendEmail(message);
    ...
    @Override
    public ResponseEntity<?> confirmationmail(String confirmationemail) {
        ConfirmationToken token = confirmationTokenRepository.findByConfirmationToken(confirmationemail);

        if(token != null)
        {

            Fournisseur fournisseur = fournisseurRepository.findFournisseurByEmail(token.getFournisseur().getEmail());
            System.out.println("email from token " +token.getFournisseur().getEmail());
            ...
            String htmlResponse = "<!DOCTYPE html>\n" +
                    ...
                    "        <a href=\"http://localhost:4200/loginFournisseur\"><button>Retour à la connexion</button></a>\n" +
                    "    </div>\n" +
                    "</body>\n" +
                    "</html>";
            MimeMessageHelper helper;
